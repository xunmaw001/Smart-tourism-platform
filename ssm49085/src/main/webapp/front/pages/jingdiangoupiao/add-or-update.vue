	...
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(248, 176, 9, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				<view :style='{"width":"200rpx","fontSize":"28rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left"}' class="title">购票日期</view>
				<picker mode="date" :value="ruleForm.goupiaoriqi" @change="goupiaoriqiChange">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(248, 176, 9, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{ruleForm.goupiaoriqi?ruleForm.goupiaoriqi:"请选择购票日期"}}</view>
				</picker>
			</view>
			<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"rgba(248, 176, 9, 1)","margin":"0 0 10rpx 0","borderWidth":"2rpx","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
...
<script>
	...
	export default {
		...
		async onLoad(options) {
    		        this.ruleForm.goupiaoriqi = this.$utils.getCurDate();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`jingdiangoupiao`, this.ruleForm.id);
		...
		methods: {
			...
			goupiaoriqiChange(e) {
				this.ruleForm.goupiaoriqi = e.target.value;
				this.$forceUpdate();
			},
			...
			async onSubmitTap() {
				...
				if(this.ruleForm.id){
					await this.$api.update(`jingdiangoupiao`, this.ruleForm);
				}else{
					await this.$api.add(`jingdiangoupiao`, this.ruleForm);
				}
